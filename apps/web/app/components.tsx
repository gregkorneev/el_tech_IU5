...
export function Sidebar() {
  return <aside><Link className="brand" href="/"><span>ЭТ</span><b>Электротехника</b><small>2026 / 27</small></Link><nav>{nav.map(item => <Link key={item.href} href={item.href}>{item.label}</Link>)}</nav><p className="sync-note"><i />Синхронизация с Диском<br /><small>Автоматически раз в сутки</small></p></aside>;
}
